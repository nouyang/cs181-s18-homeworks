...
class KMeans(object):
    ...
    def newCenters(self, imgs, clusterAssignments):
        ...
        for k in range(self.K): #retrieve imgs that are for each cluster
            ...
            if cluster.size: #check if empty cluster
    ...
    def get_representative_images(self, D):
        ...
        for k in range(self.K):
            ...
            maxdists = mindists[::-1]
            print('class %i contains %i datapoints' % (k, len(mindists)))
            #assert len(mindists)!= 0
            if len(mindists) == 0:
                repindices = [0]*D
            else:
                repindices= [mindists[i][0] for i in range(3)]
                repindices += [maxdists[i][0] for i in range(3)]
...
plt.tight_layout()
plt.suptitle('MNIST Kmeans: %i iters,%i clusters\nNormalized Obj: %.04f' % \
             (numIters, K, KMeansClassifier.cost))
time = datetime.now().strftime('%H:%M:%S')
fname = time + '_centroid_%i_iters_%i_clusters_Obj%0.04f '% (numIters, K, KMeansClassifier.cost) + '.png'
plt.savefig(fname)
...
for k, images in reps.items():
    ...
    for r in range(D):
        ...
        #print('k', k, 'r', r, 'D', D, 'K', K, 'subplt', (r*K)+k+1)
        #plt.subplot(D, K, (r*D) + k+1)
        plt.subplot(K, D, i)
        plt.imshow(img, cmap='Greys_r')
        plt.axis('off')
plt.suptitle('MNIST Kmeans Reps: %i iters,%i clusters\nNormalized Obj: %.04f' % \
             (numIters, K, KMeansClassifier.cost))
fname = time + '_reps%i_iters_%i_clusters_Obj%0.04f '% (numIters, K, KMeansClassifier.cost) + '.png'
plt.savefig(fname)
