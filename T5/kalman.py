...
# aka z kind of dithers around the same point 
# and x is a noisy measurement of z 
mean_eps = 0
var_eps = 0.05**2
mean_y = 0
var_y = 1**2
...
for step in t:
    ...
    mean, var = update(meas_mean, meas_var, post_mean, post_var)
    mean, var = predict(mean, var, mean_eps, var_eps)
    post_mean, post_var = mean, var
    est_z = 0 #np.random.normal(post_mean, post_var)

    zs.append(post_mean)
    zs_var.append(post_var)
    print("measured %0.02f, post mean %0.02f, post var %0.02f, z_est %.02f, true_z %0.02f" % \
          (meas, post_mean, post_var, est_z, z[step]))
