...
\subsection*{Solution Bayesian Networks}
    ...
    \begin{itemize}
        ...
        \begin{figure}[H]
                ...
                \includegraphics[width=\textwidth,height=0.8\paperheight]{Part1_D-sep_diagrams.png}
                \caption{Part 1. D-Separation Work}
                \label{d-sep}
        \end{figure}
...
\noindent In this problem we will explore the calculation of the \emph{MDP value function} $V$  in a 2D exploration setting, without time discounting and for a finite time horizon.  %
Consider a robot navigating the following grid:
  \begin{center}
			  \begin{tikzpicture}[darkstyle/.style={circle,draw,fill=gray!40,minimum size=20}]
				  ...
				  \end{tikzpicture}
			  \end{center}
			  The robot moves in exactly one direction each turn (and must
			  move). The robot's goal is to maximize its score in the game after $T$ steps.
			  ...
			  \begin{enumerate}
                \item Model this as a Markov decision process: define the states $S$, actions $A$,
                    reward function $r:S\times A\mapsto \mathbb{R}$, and transition model
                    $p(s'\given s, a)$ for $s', s\in S$ and $a\in A$.  For now, assume that the
                    robot's actions execute perfectly: if the robot tries to move in a particular
					direction, it always succeeds in doing so.
...
\subsection*{A solution section}
    ...
    \begin{itemize}
        ...
        \begin{figure}[H]
            \centering
            %\includegraphics[width=\textwidth,height=0.8\paperheight]{'asdffile.png'}
