...
\begin{enumerate}
    ...
    \begin{answer}
       ...
       Combining the two equations, we may write that 
        ...
        the distribution of $y_k$, is simply the number of datapoints in that
        $k$ class ($N_k$) over the total number of datapoints($n$).
 ...
 \end{answer}
  \item[6.] Derive the maximum likelihood estimator of the covariance matrix.
      ...
      covariance matrix. 
      % todo: what exactly do the two summations do? how does this not just
      % result in a scalar, but results in a meaningful error term describing
      % the relationship betweem different dimensions?
      \end{answer}
...
\begin{enumerate}
    ...
    \begin{answer}
            ...
            \centering 
            \subfloat[]{%
                \includegraphics[width=0.5\linewidth]{log_tex.png}
                \label{fig:log}%
            }\hfil % MUST be right above next figure
            \subfloat[]{%
                \includegraphics[width=0.5\linewidth]{gen_covar_tex.png}
            \label{fig:b}%
            }
            \subfloat[]{%
            \includegraphics[width=0.5\linewidth]{gen_covar_shared_tex.png}%
            \label{fig:c}%
            }
            ...
            The logistic regression does a decent job of seperating the greens
            out, aka the lemons. However, it falters where we have the blue and
            red (aka oranes and apples) essentially intermised on top of each
            other. It is unable to distinguish between them and classifies them
            all as apples.

            The generative model with \textbf{shared} covariances seems to
            struggle as well. There may be an error in my code, as it appears to
            have managed to incorrectly classify all reds (apples), calling them
            all blues (oranges), bug again, with the shared covariance Gaussian,
            we are only able to have linear decision boundaries and so the model
            has trouble distinguishing between oranges and apples. If we refer
            to the section 4 notes, we see that maximizing the posterior results
            in a linear term instead of a quadratic as in the separate
            covariance case.

            Most notably, the Generative model using multiple Gaussian
            distributions with \textbf{separate} covariances, is able to achieve a
            non-linear fit. This is because with a different covariance per
            class,  the log likelihood contains a quadratic term and we are able
            to a decision boundary as pictures in Fig 4 (b).
    \end{answer}


    \item[2.] Logistic regression hyperparameters

    \begin{answer}
            \begin{figure}[H]
            \centering 
            \subfloat[]{%
                \includegraphics[width=0.4\linewidth]{eta_10_lam_0.png}
                \label{fig:log}%
            }
            \subfloat[]{%
                \includegraphics[width=0.4\linewidth]{eta_1_lam_0.png}
            \label{fig:b}%
            }\hfil % MUST be right above next figure
            \subfloat[]{%
            \includegraphics[width=0.4\linewidth]{eta_0-01_lam_0_600.png}%
            \label{fig:c}%
            }
            \subfloat[]{%
            \includegraphics[width=0.4\linewidth]{eta_10_lam_0-0001.png}%
            \label{fig:c}%
            }\hfil % MUST be right above next figure
            \subfloat[]{%
            \includegraphics[width=0.4\linewidth]{eta_0-10_lam_0.png}%
            \label{fig:c}%
            }
            ...
            figure (e)
            Briefly, having $\eta$ equal to 0.1 and $\lambda$ kept extremely
            small (1 e-6) allowed for fast convergence and stable convergence.

            figure (d)
            With $\lambda$ values any greater than 1 e-6, the logistic
            regression model fails to converge. I hypothesize that, this is
            because the apples and oranges are so intermixed, that we must have
            relatively large weights and can't penalize the model for complexity
            too much. %does this really make sense ?

            figure (c)
            If $\eta$ is too small, say 0.01, we see that it takes much longer
            to converge -- at 200 samples, it has higher error rate than the
            $\eta$ = 0.1 case.

            figure (a) and (b)
            If $\eta$ is too big, say 1.0 or 10.0, we see that the model fails
            to converge on a low error rate.% not too clear on this.


    \end{answer}

    \item[3.] Gaussian Log Likelihood 
    \begin{answer}

        The final log likelihood achieved by the Gaussian with separate
        covariance:
        \\

        The final log likelihoods achieved by the Gaussian with shared
        covariance:
    \end{answer}
